...
/// For 2in13 EPD, WIDTH=104, HEIGHT=212.
...
impl<S: DisplaySize> FrameBuffer<S> {
    fn new() -> Self {
        ...
        buf.as_mut_slice().fill(0xff); // filled with WHITE
        Self {
            buf ,
    ...
    fn set_pixel(&mut self, x: usize, y: usize, pixel: bool) {
        ...
        let addr = y * width_in_byte + x / 8;
        if pixel {
            self.buf.as_mut_slice()[addr] &= !(0x80 >> (x % 8));
        } else {
            self.buf.as_mut_slice()[addr] |= 0x80 >> (x % 8);
        }
    }
    ...
    SIZE: DisplaySize,
{
    ...
    pub fn init(&mut self) -> Result<(), DisplayError> {
        ...
        // MUX gate = A+1
        self.di.send_data(&[((SIZE::HEIGHT - 1) & 0xff) as u8, ((SIZE::HEIGHT - 1) >> 8) as u8])?;
    ...
    pub fn set_window(&mut self, x0: u16, y0: u16, x1: u16, y1: u16) -> Result<(), DisplayError> {
        ...
        self.di.send_command(Command::SetRamXAddressStartEndPosition as u8)?;
        self.di.send_data(&[(x0 >> 3) as u8, (x1 >> 3) as u8])?;
        self.di.send_command(Command::SetRamYAddressStartEndPosition as u8)?;
        self.di.send_data(&[
            (y0 & 0xf) as u8,
            (y0 >> 8) as u8,
            (y1 & 0xff) as u8,
    ...
    pub fn display(&mut self) -> Result<(), DisplayError> {
        ...
        for j in 0..SIZE::HEIGHT {
            ...
            for i in 0..byte_width {
                self.di.send_data(&[self.framebuffer.buf.as_slice()[j * byte_width + i]])?;
            }
        }
        self.turn_on_display()
    }
